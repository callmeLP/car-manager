...
function analysisExcel($filePath){
	...
	foreach($sheet->getRowIterator() as $row)
	{
		...
		$data['pad']=$objPHPExcel->getActiveSheet()->getCell("C".$i)->getValue(); 
		$data['carNum']=$objPHPExcel->getActiveSheet()->getCell("D".$i)->getValue(); 
		$data['type']=$objPHPExcel->getActiveSheet()->getCell("E".$i)->getValue(); 
		$data['color']=$objPHPExcel->getActiveSheet()->getCell("F".$i)->getValue(); 
		$data['subpartment']=$objPHPExcel->getActiveSheet()->getCell("G".$i)->getValue(); 
		$data['remark']=$objPHPExcel->getActiveSheet()->getCell("H".$i)->getValue(); 
		
			$sql = "INSERT INTO carowner (owner, tel, password, 
            carNum, type, color, subDepartment, remark) VALUES (
            '".$data['name']."', '".$data['tel']."', '".$data['pad']."', '".$data['carNum']."', '".$data['type']."', '".$data['color']."', '".$data['subpartment']."', '".$data['remark']."')";
